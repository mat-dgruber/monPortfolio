...
const DELIVERY_STEPS = ['Arquitetura', 'UI/UX + front-end', 'Backend + IA', 'Publicação'];
      ...
      >
        <div className="max-w-3xl space-y-7">
          ...
          <div className="space-y-5">
            <motion.h1 variants={heroItem} className="text-4xl font-semibold tracking-[-0.03em] text-white sm:text-6xl lg:text-7xl">
              Construo produtos fullstack, interfaces bem desenhadas e soluções com IA aplicada.
            </motion.h1>
            <motion.p variants={heroItem} className="max-w-2xl text-lg leading-8 text-slate-300 sm:text-xl">
              Sou Matheus Diniz Gruber, engenheiro de software fullstack que conecta arquitetura, backend, front-end, UI/UX e publicação para transformar ideias em produtos reais.
            </motion.p>
          </div>
          ...
          <motion.div variants={heroItem} className="flex flex-col gap-3 sm:flex-row">
            <motion.a
              ...
              whileHover={{ y: -2 }}
              whileTap={{ scale: 0.98 }}
              className="motion-safe-sheen inline-flex items-center justify-center gap-2 rounded-full bg-blue-400 px-5 py-3 text-sm font-semibold text-slate-950 transition hover:bg-blue-300"
            >
        ...
        <div className="space-y-5 lg:pb-4">
          ...
          >
            <div className="flex items-center gap-3 border-b border-slate-800 pb-5">
              <motion.div
                animate={shouldReduceMotion ? undefined : { boxShadow: ['0 0 0 rgba(96, 165, 250, 0)', '0 0 28px rgba(96, 165, 250, 0.22)', '0 0 0 rgba(96, 165, 250, 0)'] }}
              ...
              <div>
                ...
                <p className="text-sm text-slate-400">Fullstack, UI/UX, arquitetura e IA aplicada</p>
              </div>
            </div>
            ...
            <div className="mt-5 space-y-4">
              {PROOF_POINTS.map((point, index) => (
                ...
                >
                  <motion.span
                    animate={shouldReduceMotion ? undefined : { scale: [1, 1.45, 1] }}
          ...
          >
            <div className="flex items-center justify-between gap-4">
              <div>
                <p className="text-sm font-semibold text-white">Do conceito ao deploy</p>
                <p className="mt-1 text-sm text-slate-400">Arquitetura, experiência, código e publicação no mesmo fluxo.</p>
              </div>
            ...
            <div className="mt-5 grid gap-2 sm:grid-cols-4 lg:grid-cols-2 xl:grid-cols-4">
              {DELIVERY_STEPS.map((step, index) => (
                <motion.div
                  ...
                  whileHover={{ y: -2, borderColor: 'rgba(96, 165, 250, 0.38)' }}
                  className="rounded-2xl border border-slate-800 bg-slate-950/55 p-3"
                >
                  <span className="text-xs font-semibold text-blue-300">0{index + 1}</span>
                  <p className="mt-2 text-sm leading-5 text-slate-200">{step}</p>
                </motion.div>
              ))}
            </div>
          </motion.div>
